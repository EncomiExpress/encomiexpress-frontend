...
const ActualizarPropietario = () => {
  ...
  useEffect(() => {
    if (!usuario) {
      ...
      if (propietario) {
        setFormData({
          tipoIdentificacion: propietario.tipoIdentificacion || 'CC',
          ...
          estado: propietario.estado || 'Activo'
        })
      } else {
    ...
    setLoading(false)
  }, [usuario, navigate, id, getPropietarioById])
  ...
  const handleSubmit = (e) => {
    ...
    try {
      ...
      setError('Error al actualizar propietario')
    }
  }
    ...
    <Box sx={{ p: 4 }}>
      <Paper elevation={0} sx={{ p: 4, borderRadius: 2, border: '1px solid #e2e8f0', maxWidth: 800, mx: 'auto' }}>
        <FormHeader 
          ...
          subtitle="Modifica los datos del propietario del vehículo"
        />
        ...
        <form onSubmit={handleSubmit}>
          ...
          <FormButtonGroup justify="space-between">
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              <PrimaryButton 
                type="submit"
                children="Actualizar Propietario"
              />
            </Box>
          </FormButtonGroup>
        </form>
