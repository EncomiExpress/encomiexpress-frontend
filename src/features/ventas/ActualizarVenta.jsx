...
import {
  ...
  Button, Alert, Snackbar, TextField, Autocomplete, Dialog,
  DialogTitle, DialogContent, IconButton, CircularProgress,
} from '@mui/material'
...
import { getRutas } from '../../shared/services/ventaService.js'
...
const ESTADOS_ENCOMIENDA = [
  'pendiente de recogida', 'en recogida', 'programada',
...
const ConfirmRowChip = ({ label, value, colors }) => (
  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 2, py: 0.9, overflow: 'hidden' }}>
    ...
    <Box sx={{
      ...
      px: 1.5, py: 0.3, borderRadius: 10, fontWeight: 600, fontSize: '0.75rem',
    }}>
      {value || '—'}
    </Box>
  </Box>
...
const ActualizarVenta = ({ open, onClose, venta, onSuccess }) => {
  ...
  const [form, setForm] = useState({
    idCliente: '', numeroIdentificacion: '', nombreRemitente: '', apellidoRemitente: '',
    telefonoRemitente: '', emailRemitente: '', direccionRemitente: '',
    nombreDestinatario: '', telefonoDestinatario: '', direccionDestinatario: '',
    descripcionContenido: '', peso: '', alto: '', ancho: '', profundidad: '', valorDeclarado: '',
    idRuta: '', destinoLabel: '', fechaEstimadaEntrega: '', observaciones: '',
    estado: '', metodoPago: '', valorServicio: '', impuestos: '', total: '', estadoPago: '',
  })

  // ── Cargar rutas reales al abrir el modal ──────────────────────────────────
  useEffect(() => {
    if (!open) return
    ...
    getRutas()
      ...
      .catch(() => setRutas([]))
      .finally(() => setRutasLoading(false))
  }, [open])

  // ── Poblar formulario cuando llega la venta ────────────────────────────────
  useEffect(() => {
    if (!venta) return
    setActiveStep(0)
    setErrores({})
    ...
    // ⚠️ FIX: VentaContext ya normaliza el objeto → usar venta.destinatario (singular)
    // y venta.paquete (singular). No leer desde venta.destinatarios[0]
    const destinatario = venta.destinatario || null
    const paquete = venta.paquete || null

    // Etiqueta de ruta para mostrar en la confirmación
    const destinoLabel = venta.ruta?.destino?.ciudad
      ? `${venta.ruta.destino.ciudad} (${venta.ruta.destino.departamento})`
      : venta.ruta?.nombre || (venta.idRuta ? `Ruta ${venta.idRuta}` : '')
    ...
    const datosForm = {
      ...
      peso: paquete?.peso != null ? String(paquete.peso) : '',
      alto: paquete?.alto != null ? String(paquete.alto) : '',
      ancho: paquete?.ancho != null ? String(paquete.ancho) : '',
      profundidad: paquete?.profundidad != null ? String(paquete.profundidad) : '',
      valorDeclarado: paquete?.valorDeclarado != null ? String(paquete.valorDeclarado) : '',
      ...
      valorServicio: venta.valorServicio != null ? String(venta.valorServicio) : '',
      impuestos: venta.impuestos != null ? String(venta.impuestos) : '',
      total: venta.total != null ? String(venta.total) : '',
  ...
  const handleChange = (e) => {
    ...
    // Autorellenar remitente al cambiar cliente
    if (name === 'idCliente') {
    ...
    // Autorellenar valores al cambiar ruta
    if (name === 'idRuta') {
      const ruta = rutas.find(r => r.idRuta === parseInt(value) || r.id === parseInt(value))
      if (ruta) {
        const destinoLabel = ruta.destino?.ciudad
          ? `${ruta.destino.ciudad} (${ruta.destino.departamento})`
          : ruta.nombre || `Ruta ${ruta.idRuta || ruta.id}`
        const tarifa = parseFloat(ruta.tarifa || ruta.valorBase || 0)
        const impuestos = Math.round(tarifa * 0.10)
        setForm(prev => ({
          ...prev,
          idRuta: value,
          destinoLabel,
          valorServicio: tarifa,
          impuestos,
          total: tarifa + impuestos,
        }))
      }
      return
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Cambiar estado por separado si fue modificado
    ...
    switch (activeStep) {
      ...
      case 3:
        return (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2.5 }}>
              {/* ⚠️ FIX: rutas reales en lugar de rutasMock */}
              <FormSelect
                label="Ruta"
                name="idRuta"
                value={form.idRuta}
                ...
                helperText={rutasLoading ? 'Cargando rutas…' : errores.idRuta}
              ...
              >
                {rutasLoading
                  ...
                  : rutas.length === 0
                    ...
                    : rutas.map(r => {
                      const idKey = r.idRuta || r.id
                      const label = r.destino?.ciudad
                        ? `${r.destino.ciudad} (${r.destino.departamento})`
                        : r.nombre || `Ruta ${idKey}`
                      const tarifa = r.tarifa || r.valorBase
                      return (
                        <MenuItem key={idKey} value={idKey}>
                          {label}{tarifa ? ` — $${parseFloat(tarifa).toLocaleString()}` : ''}
                        </MenuItem>
                      )
                    })
                }
              ...
              <TextField
                fullWidth label="Fecha estimada de entrega" name="fechaEstimadaEntrega"
            ...
            <FormField label="Observaciones" name="observaciones" value={form.observaciones}
              ...
              error={errores.observaciones} inputProps={{ maxLength: 500 }} />
          </Box>
        )
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2.5 }}>
              ...
              <FormSelect label="Estado de pago" name="estadoPago" value={form.estadoPago} onChange={handleChange}>
                <MenuItem value="Pendiente">
                  <Box sx={{ display: 'inline-flex', alignItems: 'center', backgroundColor: '#FEE2E2', color: '#991B1B', px: 1.5, py: 0.3, borderRadius: 10, fontWeight: 600, fontSize: '0.75rem' }}>
                    Pendiente
                  </Box>
                </MenuItem>
                <MenuItem value="Pagado">
                  <Box sx={{ display: 'inline-flex', alignItems: 'center', backgroundColor: '#D1FAE5', color: '#065F46', px: 1.5, py: 0.3, borderRadius: 10, fontWeight: 600, fontSize: '0.75rem' }}>
                    Pagado
                  </Box>
                </MenuItem>
              </FormSelect>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              ...
              <Paper elevation={0} sx={cardSx}>
                ...
                <ConfirmRow label="Ruta" value={form.destinoLabel} />
    ...
    <Dialog open={open} onClose={() => onClose && onClose()} maxWidth="md" fullWidth
      ...
      <DialogTitle sx={{ m: 0, p: 2, pb: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${theme.palette.divider}` }}>
        <Box>
          ...
          <Typography variant="body2" color={theme.palette.text.secondary}>
            {ventaOriginal?.numeroGuia ? `Modificando guía: ${ventaOriginal.numeroGuia}` : 'Modifica los campos que necesites.'}
          </Typography>
        </Box>
        <IconButton onClick={() => onClose && onClose()} sx={{ color: theme.palette.text.secondary }}>
          <CloseIcon />
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', px: 4, py: 2.5, borderTop: `1px solid ${theme.palette.divider}` }}>
        ...
        <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
          <Button onClick={() => onClose && onClose()} disableRipple
            sx={{
