...
const RegistrarVenta = ({ open, onClose, onSuccess }) => {
  ...
  const handleChange = (e) => {
    ...
    // Autorellenar remitente al elegir cliente
    if (name === 'idCliente') {
    ...
    // Autorellenar valores al elegir ruta real
    if (name === 'idRuta') {
      const ruta = rutas.find(r => r.idRuta === parseInt(value) || r.id === parseInt(value))
      if (ruta) {
        // El destino puede venir como objeto o como string según el modelo
        const destinoLabel = ruta.destino?.ciudad
          ? `${ruta.destino.ciudad} — ${ruta.destino.departamento}`
          : ruta.destino?.nombre || ruta.nombre || String(ruta.idRuta || ruta.id)
    ...
    switch (activeStep) {
      ...
      case 3:
        return (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2.5 }}>
              {/* ⚠️ FIX: selector de rutas reales en lugar de rutasMock */}
              <FormSelect
                label="Ruta"
                name="idRuta"
                value={form.idRuta}
              ...
              <TextField
                ...
                helperText={errores.fechaEstimadaEntrega}
