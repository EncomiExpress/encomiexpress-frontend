...
const ListarDestino = () => {
    ...
    const handleToggleHabilitado = (id) => {
        ...
        setSnackbar({
            open: true,
            message: `Destino ${toggleHabilitado ? 'habilitado' : 'inhabilitado'} correctamente.`,
            severity: 'success',
        })
    }
                                            ...
                                            <Box sx={{ display: 'flex', gap: 0.5 }}>
                                                ...
                                                {tienePermiso(PERMISOS.ACTUALIZAR_DESTINO) && (
                                                    <Tooltip title={destino.habilitado ? 'Inhabilitar' : 'Habilitar'}>
                                                        <IconButton
                                                            size="small"
                                                            onClick={() => handleToggleHabilitado(destino.idDestino)}
                                                            sx={{ color: theme.palette.text.primary, '&:hover': { backgroundColor: theme.palette.primary.light } }}
                                                        >
                                                            {destino.habilitado ? <CheckBoxIcon sx={{ fontSize: 18, color: theme.palette.primary.main }} /> : <CheckBoxOutlineBlankIcon sx={{ fontSize: 18, color: theme.palette.status.disabled2.color }} />}
                                                        </IconButton>
                                                    </Tooltip>
                                                )}
                                            </Box>
                    ...
                    <Box sx={{ display: 'flex', gap: 2 }}>
                        ...
                        <Paper elevation={0} sx={{ borderRadius: 2, p: 3, border: `1px solid ${theme.palette.divider}`, backgroundColor: 'white', flex: 1 }}>
                            ...
                            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
                                ...
                                <Box sx={{ gridColumn: '1 / -1' }}><Typography variant="caption" color={theme.palette.text.secondary} fontWeight={600}>Estado</Typography><Typography variant="body2" fontWeight={500} color={destinoVer.habilitado ? '#2E7D32' : '#ef4444'}>{destinoVer.habilitado ? 'Habilitado' : 'Inhabilitado'}</Typography></Box>
                            </Box>
                        </Paper>
                    </Box>
